...
public class SplineInterpolator : MonoBehaviour
{
    ...
    void Start()
    {
		TotalStep = (MappingParameter - Speed) * 100;
	}
	...
	void Update()
	{
		...
		if (mState != "Stopped")
		{
			...
			// Calculate the t param between 0 and 1
			...
			//Move the target step by step along the curve
			if (Counter <= TotalStep)
			{
				float currTime = Counter * (1.0f/TotalStep) * 10;
				...
				if (mState == "Loop" && Counter > TotalStep)
				{
						Counter = 1;
				}
